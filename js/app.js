(function () {
   ...
   * de l'émetteur vers le récepteur. Chaque bit "1" est une particule
   * lumineuse nette (cyan, occasionnellement violette) ; chaque bit "0"
   * une particule discrète et atténuée — le flux de données reste visible
   * en continu plutôt qu'un simple clignotement.
   */
  function spawnParticles(binary) {
    return new Promise(function (resolve) {
      ...
      if (prefersReducedMotion) {
        // Pas d'animation continue : un seul indicateur statique du flux actif.
      ...
      for (var i = 0; i < bits.length; i++) {
        (function (i) {
          setTimeout(function () {
            ...
            if (bits[i] === "0") {
              p.classList.add("bit-off");
            } else if (i % violetEvery === 0) {
              p.classList.add("violet");
            }
  ...
  async function runSimulation() {
    ...
    renderPipeline(1, 0);
    document.getElementById("sim-binary").textContent = binary;
    await sleep(stepDelay);
